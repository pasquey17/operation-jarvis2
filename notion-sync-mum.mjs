...
function mergeTradeImages(baseList, fromBlocks) {
  const merged = [];
  const seen = new Set();
  // Prefer page-body images first — same URL in Files carries no heading label.
    ...
    const batch = await Promise.all(
      chunk.map(async (page) => {
        ...
        try {
          ...
          console.warn(`[notion-sync-mum] page body images ${page?.id}: ${msg}`);
        }
        return t;
      })
    );
...
async function upsertTradeBatch(supabaseUrl, supabaseKey, rows) {
  ...
  const columns =
    "notion_id,date,user_id,session,outcome,rr,model,notes,trade_images,notion_extras,notion_sync_source,archived,updated_at";
